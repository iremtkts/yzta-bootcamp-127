...
DEFAULT_MODEL_A_PATH = BASE_DIR / "models" / "best_a.pt"
DEFAULT_MODEL_B_PATH = BASE_DIR / "models" / "best_b.pt"
...
MODEL_A_URL = os.getenv("MODEL_A_URL")  # opsiyonel
MODEL_B_URL = os.getenv("MODEL_B_URL")  # opsiyonel
...
async def http_download(url: str, out_path: pathlib.Path, timeout: float = 180.0):
...
async def fetch_image(url: str, timeout: float = 20.0) -> Image.Image:
    if url.startswith("data:image/"):
        ...
        return pil_from_bytes(raw, ctype)
    ...
    return pil_from_bytes(r.content, ctype)
...
async def startup():
    ...
    if MODEL_A_URL: await http_download(MODEL_A_URL, MODEL_A_PATH)
    if MODEL_B_URL: await http_download(MODEL_B_URL, MODEL_B_PATH)
